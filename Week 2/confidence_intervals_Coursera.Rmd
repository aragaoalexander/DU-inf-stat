...
```{r set-seed}
set.seed(666)                 # make sure to change the seed
```
...
samp %>%
  ...
  geom_histogram()+
  theme_minimal()
#by checking the histogram, we see that the sample distribution is somewhat
#symmetric, so we can assume that the area mean is a typical value for the
#distribution.
...
<li> **(X) The sample size, 60, is less than 10% of all houses.** </li>
...
<li> **(X) 95% of random samples of size 60 will yield confidence intervals that contain the true average area of houses in Ames, Iowa.** </li>
...
ifelse(params$mu > samp_ci$lower & params$mu < samp_ci$upper, "Yes, mu is contained in CI", "No, mu is outside CI")
...
#Calculating proportion of CI containing mu
mu_prop <- nrow(filter(ci,ci$capture_mu == "yes"))/nrow(ci)
mu_prop
...
**R:** The proportion is 96%. It's not exactly the confidence level because we're dealing with integers.
</div>
