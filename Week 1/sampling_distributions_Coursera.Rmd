...
sample_means50 %>%
    summarise(mean = mean(x_bar), med = median(x_bar), 
            sd = sd(x_bar), iqr = IQR(x_bar),
            min = min(x_bar), max = max(x_bar),
            q1 = quantile(x_bar, 0.25),  # first quartile, 25th percentile
            q3 = quantile(x_bar, 0.75))  # third quartile, 75th percentile
...
sample_means150_2000 <- ames %>%
                    rep_sample_n(size = 150, reps = 2000, replace = TRUE) %>%
                    summarise(x_bar = mean(price))

sample_means150_2000 %>%
  summarise(mean = mean(x_bar))
